    ...
    <div class="w-75 mt-4">
                ...
                @foreach($orders as $index => $order)
                    <div href="#" class="order__row {{ $order->order_status == 2 ? 'opacity-50' : null }} d-flex flex-wrap justify-content-center flex-lg-nowrap align-items-center pt-3 ps-3 pe-3 m-2 text-decoration-none">
                        ...
                        <div class="order__information w-100 h-100">
                            ...
                            <div class="">
                                <a href="{{ route('admin.users.orders.detail', [$user->id, $order->id]) }}" class="btn btn-primary w-100 btn-lg">Посмотреть детали</a>
                            </div>
                            <p class="text-center fs-6 pt-3 mb-2">
                                @if($order->order_status == 0)
                                    Заказ размещен
                                @elseif($order->order_status == 1)
                                    Идёт доставка заказа
        ...
        @else
            <h3 class="h3 pt-5 pb-5 text-dark text-center">
                На данный момент у пользователя нет заказов.
            </h3>
        @endif
    </div>
@endsection
