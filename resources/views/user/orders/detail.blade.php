            ...
            <div class="orders_offer">
                <div class="order-details__body">
                    @if($order->order_status == 0)
                        @if(\App\Models\OfferForDelivery::where('order_id', $order->id)->get()->count())
                            ...
                            @foreach($order->offers as $offer)
                                <div class="mb-3 p-3">
                                    @if($offer->message)
                                        <a class="text-decoration-none d-inline-flex align-items-center" href="{{ route('user.detail.show', $offer->user->id) }}">
                    ...
                    @elseif($order->order_status == 1)
                        <div class="p-3">
                            <a class="text-decoration-none d-inline-flex align-items-center mb-3" href="{{ Auth::user()->isAdmin() ? route('admin.users.detail', $order->delivery->user->id) : route('user.detail.show', $order->delivery->user->id) }}">
